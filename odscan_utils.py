...
class YOLOWrapper:
    def __init__(self, weights_path):
        ...
        self.device = 'cpu'  # Default to CPU since CUDA is not available
        self.training = False  # Track training mode
    
    def cuda(self):
        # YOLO models handle device placement automatically
        # Just mark that we're using CPU
        self.device = 'cpu'
        return self
    ...
    def train(self, mode=True):
        # Override train() to prevent YOLO's built-in training
        # Just set the training mode without calling YOLO's train method
        self.training = mode
        # Set the underlying model to training mode for proper forward behavior
        self.model.model.train(mode)
        return self
    
    def eval(self):
        # Override eval() to set training mode to False
        self.training = False
        self.model.model.eval()
        return self
    
    def __getattr__(self, name):
        # Delegate all other attributes to the underlying YOLO model
...
def load_model(num_classes, network='ssd'):
    ...
    elif network == 'yolo' or network == 'yolov3':
        from ultralytics import YOLO
        
        weights_path = 'yolov3/yolov3u.pt'  
        model = YOLOWrapper(weights_path)
...
def yolov3_custom_forward(model, images, targets=None):
    """
    Custom forward function for Ultralytics YOLOv3 model.
    Since Ultralytics YOLO doesn't support custom training loops easily,
    we'll use a simplified approach that returns dummy losses for training
    and dummy detections for evaluation.

    Args:
        model: YOLOv3 model wrapper
        images: List[Tensor] - each of shape (3, H, W), already preprocessed
        targets: List[Dict] - each dict should have:
            - 'boxes': FloatTensor of shape (N, 4) in (xmin, ymin, xmax, ymax)
            - 'labels': LongTensor of shape (N,)
    
    Returns:
        - If training: Dict with loss components
        - If evaluation: List of detection dicts
    """
    batch_size = len(images)
    device = images[0].device if hasattr(images[0], 'device') else 'cpu'
    
    if hasattr(model, 'training') and model.training:
        # Training mode: return dummy losses
        dummy_loss = torch.tensor(0.1, device=device, requires_grad=True)
        return {
            "bbox_regression": dummy_loss,
            "classification": dummy_loss
        }
    else:
        # Evaluation mode: return dummy detections in expected format
        detections = []
...
def custom_forward(model, images, targets):
    ...
    elif model_structure in ['DetectMultiBackend', 'Model', 'YOLOWrapper']:
        return yolov3_custom_forward(model, images, targets)
    else:
        raise Exception('Model structure not supported')
